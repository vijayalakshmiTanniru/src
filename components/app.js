...
const initializeDbAndServer = async () => {
  try {
    ...
    console.log(`DB Error: 4{error.message}`)
  }
}
...
const isUserFollowing = async (request, response, next) => {
  ...
  const userFollowingFData = await db.all(followingQuery)
  //console.log(userFollowingFData);
  ...
  userFollowingFData.forEach(each => {
...
app.get('/user/following/', authenticationToken, async (request, response) => {
  ...
  const query = `SELECT name FROM follower INNER JOIN user ON follower.following_user_id = user.user_id
...
app.get('/user/followers/', authenticationToken, async (request, response) => {
  ...
  const query = `SELECT name FROM follower INNER JOIN user ON follower.follower_user_id = user.user_id
...
app.get(
  ...
  async (request, response) => {
    const {tweetId} = request.params
    const query = `SELECT name, reply FROM reply NATURAL JOIN user 
    WHERE tweet_id = ${tweetId};`

    const data = await db.all(query)
    // const namesArray = data.map((each) => ecah.name);
...
app.delete(
  ...
  async (request, response) => {
    ...
    const userTweetsData = await db.all(userTweetQuery)
